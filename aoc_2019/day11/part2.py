...
position = Coords(0, 0)
colors: dict[Coords, t.Literal[0, 1]] = {position: 1}
...
for y in range(max(c.y for c in colors) + 1):
    for x in range(max(c.x for c in colors) + 1):
        c = Coords(x, y)
        if c in colors:
            print("#" if colors[c] == 1 else " ", end="")
        else:
            print(" ", end="")
    print()
